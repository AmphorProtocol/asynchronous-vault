...
contract TestDeposit is TestBase {
    function test_GivenVaultClosedWhenDeposit() external {
        ...
        closeVaults();
        // todo
        depositRevert(vaultUSDC, user1, 1, abi.encodeWithSelector(SyncSynthVault.ERC4626ExceededMaxDeposit.selector, user1.addr, 1, 0));
    }

    function test_GivenAmountHigherThanOwnerAllowanceToTheVaultWhenDeposit() external {
        // it should revert with ERC20InsufficientAllowance
        depositRevert(vaultUSDC, user1, IERC20(vaultUSDC.asset()).allowance(user1.addr, address(vaultUSDC)) + 1);
    }

    function test_GivenAmountHigherThanOwnerBalanceWhenDeposit() external {
        usersDealApprove(1); // vault should not be empty
        depositRevert(vaultUSDC, user1, IERC20(vaultUSDC.asset()).balanceOf(user1.addr) + 1);
    }
