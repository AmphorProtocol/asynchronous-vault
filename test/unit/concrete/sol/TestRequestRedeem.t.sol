...
contract TestRequestRedeem is TestBase {
    ...
    function test_WhenRequestRedeemSucceed() external {
        usersDealApproveAndDeposit(vaultTested, 1);
        assertClose(vaultTested);
        assertRequestRedeem(vaultTested, user1.addr, user1.addr, user1.addr, 56, "");
    }
    ...
    function test_GivenOwnerHasClaimableBalanceButNotReceiverWhenRequestRedeem() external {
        ...
        usersDealApproveAndDeposit(vaultTested, 1);
        assertClose(vaultTested);
        assertRequestRedeem(vaultTested, user1.addr, user1.addr, user1.addr, 1, "");
        assertOpen(vaultTested, 0);
        assertClose(vaultTested);
        usersDealApprove(vaultTested, 4);
        assertRequestRedeem(vaultTested, user1.addr, user1.addr, user4.addr, 1, "");
    }
