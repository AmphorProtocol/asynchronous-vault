...
contract TestRequestRedeem is TestBase {
    ...
    function test_WhenRequestRedeemSucceed() external {
        usersDealApproveAndDeposit(1);
        assertClose(vaultUSDC);
        assertRequestRedeem(vaultUSDC, user1.addr, user1.addr, user1.addr, 56, "");
    }
    ...
    function test_GivenOwnerHasClaimableBalanceButNotReceiverWhenRequestRedeem() external {
        ...
        usersDealApproveAndDeposit(1);
        assertClose(vaultUSDC);
        assertRequestRedeem(vaultUSDC, user1.addr, user1.addr, user1.addr, 1, "");
        assertOpen(vaultUSDC, 0);
        assertClose(vaultUSDC);
        usersDealApprove(4);
        assertRequestRedeem(vaultUSDC, user1.addr, user1.addr, user4.addr, 1, "");
    }
