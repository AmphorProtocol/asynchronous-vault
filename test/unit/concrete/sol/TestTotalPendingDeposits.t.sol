...
contract TestTotalPendingDeposits is TestBase {
    function test_GivenVaultOpenWhenTotalPendingDeposits() external {
        ...
        assertRequestDeposit(vaultUSDC, user1.addr, user1.addr, user1.addr, 100, "");
        assertOpen(vaultUSDC, 0);
        assertEq(vaultUSDC.totalPendingDeposits(), 0, "Invalid total pending deposits");
    }
    ...
    function test_GivenVaultClosedWhenTotalPendingDeposits() external {
        ...
        assertClose(vaultUSDC);
        assertRequestDeposit(vaultUSDC, user1.addr, user1.addr, user1.addr, 100, "");
        assertEq(vaultUSDC.totalPendingDeposits(), 100, "Invalid total pending deposits");
    }
}
