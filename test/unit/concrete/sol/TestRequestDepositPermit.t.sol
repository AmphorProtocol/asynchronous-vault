...
contract SyntheticPermit is TestBase {
    ...
    function test_depositWithPermit() public {
        ...
        assertDepositWithPermit(vaultTested, user1.addr, user1.addr, 1000 * 1e6, _deadline);
    }

    function test_depositWithPermitWithReceiverDiffMsgSender() public {
        usersDeal(vaultTested, 1);

        assertDepositWithPermit(vaultTested, user1.addr, user2.addr, 1000 * 1e6, _deadline);
    }
    ...
    function test_givenVaultClosed_depositWithPermit() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = _signPermit(user1, address(vaultTested), 1, underlyingPermit.nonces(user1.addr), _deadline);

        vm.startPrank(user1.addr);
        PermitParams memory params = PermitParams({ value: 1, deadline: _deadline, v: v, r: r, s: s });
        vm.expectRevert(abi.encodeWithSelector(SyncSynthVault.ERC4626ExceededMaxDeposit.selector, user1.addr, 1, 0));
