...
contract TestWithdraw is TestBase {
    function test_GivenVaultClosedWhenWithdraw() external {
        ...
        close(vaultTested);
        withdrawRevert(vaultTested, user1, 1, abi.encodeWithSelector(SyncSynthVault.ERC4626ExceededMaxWithdraw.selector, user1.addr, 1, 0));
    }
    ...
    function test_GivenVaultPausedWhenWithdraw() external {
        ...
        pause(vaultTested);
        withdrawRevert(vaultTested, user1, 1, PausableUpgradeable.EnforcedPause.selector);
    }
    ...
    function test_GivenReceiverIsAddress0WhenWithdraw() external {
        ...
        withdrawRevert(vaultTested, address0, 1, abi.encodeWithSelector(SyncSynthVault.ERC4626ExceededMaxWithdraw.selector, 0x0, 1, 0));
    }
    ...
    function test_GivenAssetsIsHigherThanOwnerSharesBalanceConvertedToAssetsWhenWithdraw() external {
        ...
        withdrawRevert(
            ...
            abi.encodeWithSelector(SyncSynthVault.ERC4626ExceededMaxWithdraw.selector, user1.addr, vaultTested.convertToAssets(vaultTested.balanceOf(user1.addr)) + 1, vaultTested.convertToAssets(vaultTested.balanceOf(user1.addr)))
        );
    }
    ...
    function test_GivenSenderNotOwnerAndAllowanceOfSenderForOwnerIsLowerThanWithdrawAmountWhenWithdraw() external {
        ...
        withdrawRevert(vaultTested, user1, user2, 1, abi.encodeWithSelector(IERC20Errors.ERC20InsufficientAllowance.selector, user2.addr, 0, 1));
    }
