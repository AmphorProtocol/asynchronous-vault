...
abstract contract Assertions is EventsAssertions {
    ...
    function assertWithdraw(IERC4626 vault, address sender, address owner, address receiver, uint256 assets) public {
        ...
        assertTransferEvent(vault, owner, address(0), previewedShares); // transfer
            // from vault to receiver of its shares
        assertWithdrawEvent(vault, sender, owner, receiver, assets, previewedShares);
        ...
        // assertion on shares value in assets
        assertSharesValueInAssets(vault, receiver, sharesValueBefore.receiver + assets);
    }
    ...
    function assertRedeem(IERC4626 vault, address sender, address owner, address receiver, uint256 shares) public {
        ...
        SharesValueData memory sharesValueBefore = getSharesValueData(vault, sender, owner, receiver);
        ...
        // assertion on shares value in assets
        assertSharesValueInAssets(vault, receiver, sharesValueBefore.receiver + previewedAssets);
    }
    ...
    function assertOpen(AsyncSynthVault vault, int256 performanceInBps) public {
        ...
        assertWithdrawEvent(vault, address(vault.pendingSilo()), address(vault.claimableSilo()), address(vault.pendingSilo()), expectedAssetsToRedeem, stateBefore.pendingRedeem);
    ...
    function assertAssetBalance(IERC4626 vault, address owner, uint256 expected) public {
        ...
        string memory vaultLabel = vm.getLabel(address(vault));
        string memory explanation = " | Current (left) != Expected (right)";
        assertEq(IERC20(vault.asset()).balanceOf(owner), expected, string.concat(userLabel, " has wrong asset balance in ", vaultLabel, explanation));
    }
