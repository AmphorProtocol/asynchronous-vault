...
contract SynthVaultRequestReceipt is ERC6909ib, Ownable {
    ...
    constructor(ERC20 _underlying, string memory name, string memory symbol)
        ERC6909ib(name, symbol)
        Ownable(_msgSender())
    {
    ...
    function deposit(
        ...
        require(
            (shares = previewDeposit(epochNonce, assets)) != 0, "ZERO_SHARES"
        );
    ...
    function burn(address account, uint256 tokenId, uint256 shares)
        external
        onlyOwner
    {
        if (balanceOf[account][tokenId] >= shares) {
    ...
    function nextEpoch(uint256 currentEpochNonce)
        external
        onlyOwner
