...
abstract contract SyncSynthVault is IERC4626, Ownable2StepUpgradeable, ERC20PermitUpgradeable, ERC20PausableUpgradeable {
    /*
     * ####################################
    ...
    */
    ...
    function initialize(uint16 fees, address owner, IERC20 underlying, string memory name, string memory symbol) public virtual onlyInitializing {
    ...
    * @dev The `maxDeposit` function is used to calculate the maximum deposit.
    ...
    function maxWithdraw(address owner) public view returns (uint256) {
        return vaultIsOpen && !paused() ? _convertToAssets(balanceOf(owner), Math.Rounding.Floor) : 0;
    }
    ...
    */
    ...
    function restruct(uint256 virtualReturnedAsset) external onlyOwner {
        ...
        emit EpochEnd(block.timestamp, _totalAssets, virtualReturnedAsset, 0, totalSupply());
    ...
    */
     ...
     * assets amount.
     */
    function depositWithPermit(uint256 assets, address receiver, PermitParams calldata permitParams) external returns (uint256) {
    ...
    function execPermit(address owner, address spender, PermitParams calldata permitParams) internal {
        ERC20Permit(address(_asset)).permit(owner, spender, permitParams.value, permitParams.deadline, permitParams.v, permitParams.r, permitParams.s);
    }
    ...
    */
    ...
    function depositWithPermit2(uint160 assets, address receiver, IAllowanceTransfer.PermitSingle calldata permitSingle, bytes calldata signature) external whenClosed whenNotPaused returns (uint256) {
        execPermit2(permitSingle, signature);
        PERMIT2.transferFrom(_msgSender(), address(this), assets, address(_asset));
    ...
    // using Permit2.
    function execPermit2(IAllowanceTransfer.PermitSingle calldata permitSingle, bytes calldata signature) internal {
